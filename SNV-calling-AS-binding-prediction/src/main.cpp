...
int main(int argc,char *argv[])
{
	if(argc<12)
	{
		...
		cout<<"Version: 0.1\n";
		cout<<"Usage: SNVAS -bed <peaks.bed> -bam <peaks.bam> -controlbam <controlpeaks.bam> -o <output.vcf> -fermi <fermi> -tmp <tmp>\n\n";
	...
	else
	{
		...
		const string OutputVcffile(argv[8]);
		const string fermi_location(argv[10]);
		const string tmpfilefolder(argv[12]);
...
cout<<"finish read input bam"<<endl;
		...
		ReadBamfile(peakbedregion_set,Bamfile,AllPeakInputBamInfor,fermi_location,tmpfilefolder,OutputVcffile);
cout<<"finishall"<<endl;
	}
}
