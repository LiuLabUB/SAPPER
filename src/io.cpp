...
void ReadPeakBedFile(const string Peakbedfile,const string Bamfile,vector<BedRegion> &peakbedregion_set)
{
  ...
  if(fp==NULL) {cerr<<"Error: Can't read BAM file with gzopen: "<<Bamfile<<endl;exit(0);}
  ...
  if(!is) {cerr<<"Error: Can't open: "<<Peakbedfile<<endl;exit(0);}
...
void GetRefSeq(const uint8_t *seq,const int32_t l_seq,const char *qual,string &cigar,string &MD_str,string &revisedseq,string &revisedbq,int &readend,string &refseq)
{
  ...
  for(unsigned long i=0;i<cigar.length();i++)
	...
	{
	  itemp=i;
	  if(i==0) {cerr<<"Error: error1:"<<cigar<<endl;exit(0);}
	}
    }
      ...
      if(cigar[i]<'0' || cigar[i]>'9')
	{
	  ...
	  if(!stemp.empty())
	      ...
	      else if(cigar[i]=='M')//match or mismatch
		{
	      ...
	      else {cerr<<"Error: error reading "<<cigar<<endl;exit(0);}
	    }
	}
    }

  //
  if(MD_str[0]!='Z') {cerr<<"Error: Can't get correct MD info in read: "<<MD_str<<endl;exit(0);}
      ...
      if(cigar[i]<'0' || cigar[i]>'9')
	{
	  ...
	  if(!stemp.empty())
	      ...
	      else if(cigar[i]=='M')//match or mismatch
		{
	      ...
	      else {cerr<<"Error: reading cigar "<<cigar<<endl;exit(0);}
	    }
	}
    }
  ...
  for(unsigned long i=0;i<MD_str.length();i++)
    {
      if(MD_str[i]=='^')
	{
	  if(i==0) {cerr<<"Error: Wrong MD flag: "<<MD_str<<endl;exit(0);}
	  else
	    {
	      end=i-1;
  ...
  do
    {
      ...
      //
      if(chrindex2name.find(refID)==chrindex2name.end()) {cerr<<"Error: Can't get correct refID: "<<refID<<endl;exit(0);}
...
void ReadInputBamfile(const vector<BedRegion> &peakbedregion_set,const string InputBamfile,vector<vector<BamInfor> > &AllPeakInputBamInfor)
{
  ...
  if(fp==NULL) {cerr<<"Error:Can't read BAM file using gzopen:"<<InputBamfile<<endl;exit(0);}
  ...
  do
    {
      ...
      //
      if(chrindex2name.find(refID)==chrindex2name.end()) {cerr<<"Error:Can't get refID: "<<refID<<endl;exit(0);}
      ...
      if(chrindex2name[refID]==peakchr)
	{
	  if(readend>=peakstart && readstart<=peakend) PeakInputBamInfor.push_back(mybaminfor);
	      ...
	      do
		{
		  ...
		  else {cerr<<"Error: Can't put read in the next region"<<readstart<<" "<<peakstart<<"-"<<peakend<<endl;exit(0);}

		}while(1);
	    }
	  else {cerr<<"Error: Reads are not sorted by coordinates: "<<readstart<<" "<<peakstart<<"-"<<peakend<<endl;exit(0);}
	}
      else
	{
	  AllPeakInputBamInfor.push_back(PeakInputBamInfor);
      ...
      if(chrindex2name[refID]==peakchr)
	{
	  if(readend>=peakstart && readstart<=peakend) PeakBamInfor.push_back(mybaminfor);
	      ...
	      do
		{
		  ...
		  else if(readstart<peakstart) {cerr<<"Error: Can't put read in the next region!!"<<readstart<<" "<<peakstart<<"-"<<peakend<<endl;exit(0);}

		}while(1);
	    }
	  else {cerr<<"Error: BAM is not sorted by coordinates: "<<readstart<<" "<<peakstart<<"-"<<peakend<<endl;exit(0);}
	}
      else
	{
	  AssembleAndSNVAS(top2nt_minpercent,Fermi_overlap_minpercent,ReadLength,fermi_location,tmpfilefolder,OutputVcffile,PeakIndex,peakbedregion_set[PeakIndex].chr,PeakBamInfor,AllPeakInputBamInfor[PeakIndex]);
	  ...
	  do
	      ...
	      if( (PeakIndex+1) %PEAK_INFO_STEP==0)
		{
	      ...
	      if(chrindex2name[refID]!=peakchr) {cerr<<"Error: Can't put read in the next chromosome: "<<chrindex2name[refID]<<"-"<<peakchr<<" "<<readstart<<" "<<peakstart<<"-"<<peakend<<endl;exit(0);}

	      if(readend>=peakstart && readstart<=peakend) {PeakBamInfor.push_back(mybaminfor);break;}
	      else if(readstart<peakstart) {cerr<<"Error: Can't put read in the new chromosome!!: "<<readstart<<" "<<peakstart<<"-"<<peakend<<endl;exit(0);}
  ...
  for(unsigned long i=0;i<input.size();i++)
    {
      ...
      else {cerr<<"Error: Can't get reference seq1 nt: "<<i+1<<" "<<input[i]<<endl;exit(0);}
    }
}
...
void AssembleAndSNVAS(const double top2nt_minpercent,const double Fermi_overlap_minpercent,const int ReadLength,const string fermi_location,const string tmpfilefolder,const string OutputVcffile,const int PeakIndex,const string regionchr,const vector<BamInfor> &PeakBamInfor,const vector<BamInfor> &PeakInputBamInfor)
{
  ...
  if(!os1) {cerr<<"Error: Can't open temporary output file: "<<ss1.c_str()<<endl;exit(0);}
  ...
  for(unsigned long i=0;i<PeakBamInfor.size();i++)
    {
      ...
      //
      if(mybaminfor.refseq.size()!=mybaminfor.end-mybaminfor.start+1) {cerr<<"Error: Can't read reference sequences"<<endl;exit(0);}
  ...
  //
  if(extendref_end-extendref_start+1 != extendrefseq.size()) {cerr<<"Error: Can't read reference seq for ChIP: "<<endl;exit(0);}
	  ...
	  if(pos->first != itian+1)
	    {
	      ...
	      if(itemp<0) {cerr<<"Error: Can't map index: "<<pos->first<<" "<<itian<<endl;exit(0);}
  ...
  if(extendref_end-extendref_start+1!=extendrefseq.size()) {cerr<<"Error: Can't extend ref: "<<extendref_end<<" "<<extendref_start<<" "<<extendrefseq.size()<<endl;exit(0);}

  //fermi
  string p1_mag;
  p1_mag = tmpfilefolder+"/"+to_string(PeakIndex+1)+"_p1.mag";
  string cmdstring;
  // call fermi function assemble() ; use fork() to protect main process
  pid_t child_id = fork();
  if (child_id == -1) {
    ...
    if (child_exit_status != 0) {
      // if failed, return without doing anything further
      cerr << "Assembly failed for peak " << PeakIndex << ", so this peak is skipped." << endl;
      return;
    }
  }
  ...
  if(!is) {cerr<<"Error: Can't open assembled sequence in: "<<p1_mag.c_str()<<endl;return;}
  do
    {
      sbuf.clear();
      is>>sbuf;
  ...
  for(unsigned long i=0;i<contigseq_set.size();i++)
      ...
      if(myrefseq_noinsert!=extendrefseq)
	{
	  ...
	  if(position==extendrefseq.npos) {cerr<<"Error: Mystery error!"<<endl;exit(0);}
  ...
  for(unsigned long i=0;i<contigseq.size();i++)
      ...
      for(unsigned long j=0;j<refseq[i].size();j++)
	{
      ...
      if(ia!=refend[i]) {cerr<<"Error: Contig coordinations are inconsistent: "<<ia<<" "<<refstart[i]<<" "<<refend[i]<<endl;exit(0);}
  ...
  for(unsigned long i=0;i<refstart.size();i++)
      ...
      for(unsigned long j=0;j<refseq[i].size();j++)
	{
	  ...
	  if(j==refseq[i].size()-1)
	    {
	      if(pos!=refend[i]) {cerr<<"Error: Incorrect start and end position for contig: "<<contigseq[i]<<endl;exit(0);}
	    }
...
void GetReadSeqCoor(const string seq,const string bq,const int startpos,const string cigar,string &revisedseq,string &revisedbq,vector<int> &revisedcoor)
{
  ...
  if(cigar.substr(0,2)=="0I" || cigar.substr(0,2)=="0D" || cigar.substr(0,2)=="0M") {cerr<<"Error: error1:"<<cigar<<endl;exit(0);}
      ...
      if(cigar[i]<'0' || cigar[i]>'9')
	{
	  ...
	  if(!stemp.empty())
	      ...
	      else if(cigar[i]=='M')//match or mismatch
		{
	      ...
	      else {cerr<<"Error: error:"<<cigar<<endl;exit(0);}
	    }
	}
    }
}
...
void FillContigInfor(const int snvpos,const char snvref,const vector<char> contigNTs,map<int,PosReadsInfor> &pos2Readsinfo)
{
  ...
  else {cerr<<"Error: Can't find existing snvpos: "<<snvpos<<endl;exit(0);}
}
...
void FillChIPraw(const int snvpos,const char readnt,map<int,PosReadsInfor> &pos2Readsinfo)
{
  ...
  if(pos2Readsinfo.find(snvpos)==pos2Readsinfo.end()) {cerr<<"Error: Can't find existing snvpos: "<<snvpos<<endl;exit(0);}
  ...
  else {cerr<<"Error: Found non-standard nucleotide name (not in ATCGN): "<<readnt<<endl;exit(0);}
}
...
void FillControlraw(const int snvpos,const char readnt,map<int,PosReadsInfor> &pos2Readsinfo)
{
  ...
  else {cerr<<"Error: Found non-standard nucleotide (not in ATCGN): "<<readnt<<endl;exit(0);}
}
...
void FillControlQualInfor(const int snvpos,const char readnt,const char readbq,map<int,PosReadsInfor> &pos2Readsinfo)
{
  ...
  if(pos2Readsinfo.find(snvpos)==pos2Readsinfo.end()) {cerr<<"Error: Can't find existing snvpos1: "<<snvpos<<endl;exit(0);}
  ...
  else {cerr<<"Error: Found non-standard nucleotide1 (not in ATCGN): "<<readnt<<endl;exit(0);}
}
...
void OutputVcfResultHasInput(const string outputfile,const string regionchr,map<int,PosReadsInfor> &pos2Readsinfo)
{
  ...
  if(!os) {cerr<<"Error: Can't open output file "<<endl;exit(0);}
...
void OutputVcfResultHasInput_header(const string outputfile,const int argc,char *argv[])
{
  ...
  char date_c_arr[9];
  strftime(date_c_arr,sizeof(date_c_arr),"%Y%m%d",now);
  string date_str = date_c_arr;  

  //output
  ofstream os(outputfile.c_str());
  if(!os) {cerr<<"Error: Can't open output file "<<endl;exit(0);}
  os<<"##fileformat=VCFv4.1"<<endl;
  os<<"##fileDate="<< date_str <<endl;
